...
<div class="recherche">
    <form action="index.php?a=rechercher" method="post">
        <p class="search">
            <!-- la balise select permet d'afficher un menu déroulant -->
...
if(isset($_POST) && !empty($_POST)){
    //on définit les requêtes de recherche en fonction du critère selectionné et de la reponse du formulaire
    if ($_POST['critere'] == 'contient') {
        $requete = 'SELECT * FROM pokemon WHERE nom LIKE "%' . $_POST['reponse'] . '%" ORDER BY id ASC';
    }
    ...
    elseif ($_POST['critere'] == 'id') {
        $requete = 'SELECT * FROM pokemon WHERE id ='.$_POST['reponse'].' ORDER BY id ASC';
    }
    elseif ($_POST['critere'] == 'generation') {
        $requete = 'SELECT * FROM pokemon WHERE pokemon.generation ='.$_POST['reponse'].' ORDER BY pokemon.id ASC';
    }
    elseif ($_POST['critere'] == 'type') {
        $requete = 'SELECT pokemon.id_categorie, pokemon.nom, pokemon.id, pokemon.path_to_image, pokemon.path_to_image_shiny, pokemon.taille, pokemon.poids, pokemon.attack,pokemon.pv, pokemon.defense, pokemon.vitesse FROM pokemon, type, link_type WHERE pokemon.id = link_type.id_pokemon AND link_type.id_type = type.id AND type.nom ="'.$_POST['reponse'].'" ORDER BY pokemon.id ASC';
    }
    elseif ($_POST['critere'] == 'faiblesse') {
        $requete = 'SELECT pokemon.id_categorie, pokemon.nom, pokemon.id, pokemon.path_to_image, pokemon.path_to_image_shiny, pokemon.taille, pokemon.poids, pokemon.attack, pokemon.pv, pokemon.defense, pokemon.vitesse FROM pokemon, type, link_faiblesse WHERE pokemon.id = link_faiblesse.id_pokemon AND link_faiblesse.id_type = type.id AND type.nom ="'.$_POST['reponse'].'" ORDER BY pokemon.id ASC';
    }
    elseif ($_POST['critere'] == 'categorie') {
        $requete = 'SELECT pokemon.id_categorie, pokemon.nom, pokemon.id, pokemon.path_to_image, pokemon.path_to_image_shiny, pokemon.taille, pokemon.poids, pokemon.attack, pokemon.pv, pokemon.defense, pokemon.vitesse FROM pokemon, categorie WHERE pokemon.id_categorie = categorie.id AND categorie.nom = "'.$_POST['reponse'].'" ORDER BY pokemon.id ASC';
    }
    //debug("requete",$requete);
    //on execute la requête...
    $reponse = $bdd->query($requete);
    //et on ajoute lister.php à la page pour afficher le résultat de la recherche sous forme de tableau
    include 'lister.php';
}
?>
